...
public class VUnrealistic extends Widget implements Paintable, ClickHandler {
	...
	public void updateFromUIDL(final UIDL uidl, final ApplicationConnection client) {
		...
		playStyle.setBackgroundImage("url(" + Window.Location.getPath() + "/VAADIN/themes/" + uidl.getStringAttribute("play") + ")");
		...
		if (uidl.hasAttribute("path")) {
			playStyle.setBackgroundImage("url(" + uidl.getStringAttribute("path") + "/VAADIN/themes/" + uidl.getStringAttribute("play") + ")");
		}
		...
		while (children.hasNext()) {
			...
			final VUnrealisticCell animationCell = new VUnrealisticCell(imageString, width, height, columns, rows);
			...
			while (data.hasNext()) {
				...
				if (config.getTag().equals("configuration")) {
					...
					if (config.hasAttribute("loop")) {
						...
						animationCell.setLoopStart(config.getIntAttribute("loopStartColumn"), config.getIntAttribute("loopStartRow"));
					}
					...
					if (config.hasAttribute("endColumn")) {
						animationCell.setEndFrame(config.getIntAttribute("endColumn"), config.getIntAttribute("endRow"));
					}
	...
	private DelayPosition buildDelayPosition(final UIDL delayUIDL) {
		...
		final DelayPosition delayPosition = new DelayPosition(delay, delayColumn, delayRow);
	...
	private LimitChange buildLimitChange(final UIDL limit) {
		final LimitChange change = new LimitChange(limit.getIntAttribute("delay"));
